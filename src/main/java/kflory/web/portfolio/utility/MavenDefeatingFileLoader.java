...
/*
 * I'm not able to use a ResourceLoader to access most files using relative paths of form "/x/y/z.*" or "x/y/z.*". This class encapsulates the most consistent way I've found.
 * 
 * Notes on the underlying issue:
 *  - Maven is known to make loading from the classpath at runtime difficult, as it searches for files within the WAR, and they've been mapped to different locations than in source. In my case, it looks like it's attempting to search using /target as the classpath root, but even there, results are unexplainably odd; attempts to get at, for example, "classpath:**\/*.*" has at times returned only four or five files.
 ...
 *  - This is only true in Java code. I'm not sure what mechanism is used in, say, JSP, but using paths of form /resource/static/\<imagename\> works just fine. Interestingly, this also holds true for file paths in the PropertySource annotation. 
 * @author kflory
 *
 */
